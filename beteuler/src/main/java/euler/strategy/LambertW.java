 ...
 * LambertW.java
 *
 * created at 2020-06-16 by f.grozdanov <YOURMAILADDRESS>
 *
 ...
 * specfunc/lambert.c
 * 
 * Copyright (C) 2007 Brian Gough Copyright (C) 1996, 1997, 1998, 1999, 2000, 2001 Gerard Jungman
 *
...
/*
 * Started with code donated by K. Briggs; added error estimates, GSL foo, and minor tweaks. Some
 * Lambert-ology from [Corless, Gonnet, Hare, and Jeffrey, "On Lambert's W Function".]
 */
...
public class LambertW {
  ...
  public static GSL_RETURN halley_iteration(double x, double w_initial, int max_iters,
  ...
  static final double[] c =
      {-1.0, 2.331643981597124203363536062168, -1.812187885639363490240191647568,
          ...
          12.250753501314460424, -18.100697012472442755, 27.029044799010561650};
  ...
  static GSL_RETURN gsl_sf_lambert_W0_e(double x, gsl_sf_result result) {
    ...
    if (x == 0.0) {
      ...
      /*
       * Strictly speaking this is an error. But because of the arithmetic operation connecting x
       * and q, I am a little lenient in case of some epsilon overshoot. The following answer is
       * quite accurate in that case. Anyway, we have to return GSL_EDOM.
       */
      result.val = -1.0;
      ...
      if (x < 1.0) {
        /*
         * obtain initial approximation from series near x=0; no need for extra care, since the
         * Halley iteration converges nicely on this branch
         */
  ...
  static GSL_RETURN gsl_sf_lambert_Wm1_e(double x, gsl_sf_result result) {
    if (x > 0.0) {
      ...
      if (x < -1.0e-6) {
        /*
         * Obtain initial approximation from series about q = 0, as long as we're not very close to
         * x = 0. Use full series and try to bail out if q is too small, since the Halley iteration
         * has bad convergence properties in finite arithmetic for q very small, because the
